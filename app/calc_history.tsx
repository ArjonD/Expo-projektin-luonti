...
import { Text, TextInput, View, Button, StyleSheet, FlatList, Alert, KeyboardAvoidingView, Platform } from "react-native";

export default function calc_history() {
    const [num1, setNum1] = useState("");
    const [num2, setNum2] = useState("");
    const [result, setResult] = useState<number | null>(null);
    const [equations, setEquations] = useState<{ key: string }[]>([]);
    ...
    const handleAddition = () => {
        ...
        setEquations([...equations, { key: `${parsedNum1} + ${parsedNum2} = ${sum}` }]);
    };

    const handleSubtraction = () => {
        const parsedNum1 = parseFloat(num1);
        const parsedNum2 = parseFloat(num2);
        const minus = parsedNum1 - parsedNum2;
        ...
        setResult(parsedNum1 - parsedNum2);
        setEquations([...equations, { key: `${parsedNum1} - ${parsedNum2} = ${minus}` }])
    };
